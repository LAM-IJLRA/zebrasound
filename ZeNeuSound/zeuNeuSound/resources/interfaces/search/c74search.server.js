...
const { badRequest } = __webpack_require__(1);
const Query = __webpack_require__(6);

module.exports.getAPIBaseURL = () => {
	return `${process.env.BASE_URL || `http://localhost:${process.env.HTTP_PORT}`}/api/v1`;
};
...
module.exports.ensureQuery = query => {
	return query && query instanceof Query && query.isSet;
};

module.exports.parsePageQueryArg = arg => {
	if (!arg) return 1;
	if (isNaN(arg)) throw badRequest(`${arg} is not a valid Page parameter. Expected a numerical value >= 1`);

	const p = parseInt(arg, 10);
	if (p <= 0) throw badRequest(`${arg} is not a valid Page parameter. Expected a numerical value >= 1`);
...
const Hoek = __webpack_require__(167);
...
const db = __webpack_require__(25);
const { ESCAPE_CHAR, PER_PAGE } = __webpack_require__(23);

const escapeQuotes = q => {
	return q.replace(/"/g, "'").replace(/'/g, "''");
};

const transformQueryForFTSQuery = q => {
	return escapeQuotes(q).replace(/[^a-z0-9.]/gmi, " ").trim();
};

const transformQueryForLikeQuery = q => {
	return escapeQuotes(q).replace(/%/g, `${ESCAPE_CHAR}%`).replace(/_/g, `${ESCAPE_CHAR}_`);
};

// TODO: Using a proper Grammer / Language would be better here and allow us
// to add search query language down the line more easily
class Query {

	constructor(queryText = "") {

		this._text = (queryText || "").replace(/\s+/g, " ").trim().toLowerCase();
	}

	get isSet() {
		return this._text && this._text.length && !/^\.*$/.test(this._text);
	}
	...
	get sqlEscapedMatchGroups() {
		return this._text.split(" ").map(transformQueryForFTSQuery).filter(q => q && q.length);
	}

	get sqlEscapedText() {
		return `${escapeQuotes(this._text)}`;
	}

	get sqlEscapedLowerCaseText() {
		return `${escapeQuotes(this.lowerCaseText)}`;
	}

	get allowsAutocomplete() {
		if (this._text.split(" ").length === 1) return true;
		return false;
	}

	get sqlAutocompleteText() {
		if (!this.allowsAutocomplete) throw new Error("Autcomplete is not possible on this query text");

		return transformQueryForLikeQuery(this.text);
	}

	get allowsPrefixSearch() {
		return !this.isPhraseQuery;
	}

	get isPhraseQuery() {
		if (this._text.startsWith('"') && this._text.endsWith('"')) return true;
		if (this._text.startsWith("'") && this._text.endsWith("'")) return true;
		return false;
	}

	get phrase() {
		if (!this.isPhraseQuery) throw new Error("Phrase search not supported for 'non-phrase' query.");

		let q;
		if (this._text.startsWith('"') && this._text.endsWith('"')) {
			q = this._text.slice(1, -1);
		} else if (this._text.startsWith("'") && this._text.endsWith("'")) {
			q = this._text.slice(1, -1);
		} else {
			q = this._text;
		}

		return `"${transformQueryForFTSQuery(q)}"`;
	}
...
var callSiteToString = __webpack_require__(44).callSiteToString
var eventListenerCount = __webpack_require__(44).eventListenerCount
...
module.exports.FILE_KINDS = {

	GENOBJECT_COMMON: "genobject_common",
	GENOBJECT_DSP: "genobject_dsp",
	GENOBJECT_JIT: "genobject_jit",
	GENDSP: "gendsp",
	GENJIT: "genjit",
	GENEXPR: "genexpr",
...
module.exports.localResourceCaching = (req, res, next) => {

	// Use this for local SQLite Caching. We don't wanna Cache toooooooo much here but at least
	// enough to make result drill-down, paging etc smooth enough
	//
	// 300 = 5 min
...
module.exports.remoteResourceCaching = (req, res, next) => {
	// Tell client that remote results can be cached for a while
	//
	// 600 = 10 min
...
var contentDisposition = __webpack_require__(61);
var contentType = __webpack_require__(26);
...
const { badRequest, notFound } = __webpack_require__(1);
const { FILE_KINDS } = __webpack_require__(8);

const { ESCAPE_SUFFIX, PER_PAGE } = __webpack_require__(23);
const { ensureQuery, getAPIBaseURL } = __webpack_require__(0);
const db = __webpack_require__(25);

const toColumnMatchString = (queryItems = [], column = null, prefix = false, join = null) => {
	if (!queryItems || !queryItems.length) return "";
	if (!join) join = prefix ? "OR" : "NEAR";

	const q = queryItems.map(v => {
		return `${column ? `${column}:` : ""}${v}${prefix ? "*" : ""}`;
	}).join(` ${join.trim()} `);

	if (!q || !q.length) return null;

	return queryItems.length === 1 || join === "OR" ? q : `(${q})`;
};

const buildFTSMatchString = (query, prefix) => {
	if (query.isPhraseQuery) return query.phrase;

	const splitQuery = query.sqlEscapedMatchGroups;
	if (!splitQuery || !splitQuery.length) return null;

	return `'${[toColumnMatchString(splitQuery, "_name", prefix), toColumnMatchString(splitQuery, "_tags", prefix), toColumnMatchString(splitQuery, "_digest", prefix), toColumnMatchString(splitQuery, "_desc", prefix), toColumnMatchString(splitQuery, "_filtertext", prefix, "NEAR")].join(" OR ")}'`;
};

const buildIDExcludeFilter = models => {
	if (!models || !models.length) return "";

	return `AND docid NOT IN (${models.reduce((sum, m) => {
		if (!m) return sum;
		if (m.id || m._id) sum.push(m.id || m._id);
		return sum;
	}, []).join(", ")})`;
};

const generateFileIconURL = (kind, id) => {
	if (kind === FILE_KINDS.OBJECT || kind === FILE_KINDS.FAKEOBJECT || kind === FILE_KINDS.SNIPPET || kind === FILE_KINDS.VIGNETTE // Vignettes can be a topic or a guide....
	) {
			// FileTypes with custom icon support
			return `${getAPIBaseURL()}/fileicons/${encodeURIComponent(id)}`;
		}

	return `${getAPIBaseURL()}/fileicons/kinds/${encodeURIComponent(kind)}`;
};

const parseMeta = (meta = null) => {
	if (!meta) return {};
...
const toColumnWeightsArray = (weights = {}) => {

	// Order MATTERS here in order to make sure that we apply the correct ranking value per column based on the column index..
	return ["_desc", "_digest", "_filtertext", "_kind", "_name", "_tags"].map(columnID => weights[columnID] || 0);
};

class MaxBaseModel {

	static emptyResult(page = 1, perPage = 25) {
		return {
			results: [],
			page,
			perPage,
	...
	static parse(data) {
		...
		return {
			...
			highlight: data._highlight || "",

			meta: parseMeta(data._meta),
			tags: parseTags(data._tags),

			iconURL: generateFileIconURL(data._kind, data._id),
			rank: data._rank || 0
		};
	}
	...
	constructor() {
		...
		this._kindFilter = this.constructor.KIND_FILTER ? this.constructor.KIND_FILTER : `_kind IN (${this.kinds.map(k => `'${k}'`).join(", ")})`;

		// with default attributes
		this._reqAttr = Object.freeze(["_id", "_filename", "_kind", "_meta", "_package", "_path", "_tags"].concat(this.constructor.REQ_ATTR));

		this._reqAttrList = this._reqAttr.join(", ");
		this._weightedColumns = toColumnWeightsArray(this.searchWeights);
	}
	...
	get autocompleteColumn() {
		return this.constructor.AUTOCOMPLETE_COLUMN || null;
	}
	...
	get reqAttr() {
		return this._reqAttr;
	}

	get searchWeights() {
		return this.constructor.SEARCH_WEIGHTS || {};
	}

	async autocomplete(query, page = 1, perPage = PER_PAGE) {
		if (!ensureQuery(query)) throw badRequest("Missing 'query' parameter.");

		// No autocomplete
		if (!this.autocompleteColumn || !query.allowsAutocomplete) {
			return this.constructor.emptyResult(page, perPage);
		}
		...
		const total = (await db.get(`
			...
			WHERE
				${this.kindFilter}
				AND ${this.constructor.STATUS_FILTER}
				AND ${this.autocompleteColumn} LIKE ? ${ESCAPE_SUFFIX}
			;
		`, [`%${query.sqlAutocompleteText}%`]))._total;

		const models = await db.all(`
			SELECT
				${this._reqAttrList}
				, instr(lower(${this.autocompleteColumn}), ? ) AS _matchPos
			FROM
				_things
			WHERE
				${this.kindFilter}
			...
			ORDER BY
				_matchPos ASC
				, ${this.autocompleteColumn} COLLATE NOCASE ASC
			LIMIT ?
			OFFSET ?
			;
		`, [query.sqlEscapedLowerCaseText, `%${query.sqlAutocompleteText}%`, perPage, (page - 1) * perPage]);

		return {
			results: models.map(m => this.constructor.parse(m)),
			page,
			perPage,
			total
		};
	...
	async getById(id = null) {
		...
		if (!id) throw badRequest("Missing 'id' parameter for requesting model.");

		const model = await this._db.get(`
			SELECT
				${this._reqAttrList}
			FROM
				_things
		...
		if (!model) throw notFound(`No such ${this.constructor.name} with id '${id}'`);

		return this.constructor.parse(model);
	}

	async search(query, page = 1, perPage = PER_PAGE) {
		if (!ensureQuery(query)) throw badRequest("Missing param 'query' for searching models.");
		if (!page || page <= 0) throw badRequest("Missing or invalid param 'page' for searching models.");
		if (!perPage || perPage <= 0) throw badRequest("Missing or invalid param 'perPage' for searching models.");
		...
		// include autocomplete in search
		if (this.autocompleteColumn && query.allowsAutocomplete) {
			searchQueries.push(`
				SELECT
						_id as docid
						, 10 as _rank
						, '' as _highlight
						, instr(lower(_name), ? ) AS _matchPos
					FROM
						_things
					WHERE
						${this.kindFilter}
						...
						AND ${this.autocompleteColumn} LIKE ? ${ESCAPE_SUFFIX}
			`);
			searchParams.push(query.sqlEscapedLowerCaseText); // lowercase position
			searchParams.push(`%${query.sqlAutocompleteText}%`); // LIKE query
			...
			countParams.push(`%${query.sqlAutocompleteText}%`); // LIKE query
		}

		const ftsQuery = buildFTSMatchString(query, false);
		if (ftsQuery) {
			searchQueries.push(`
				SELECT
					docid
					, (simple_rank(matchinfo(_search), ${this._weightedColumns}) * COALESCE(_factor, 1)) AS _rank
					, ${this.constructor.HIGHLIGHT ? "snippet(_search, '<b>', '</b>', '...', 2, 16) as _highlight" : "'' as _highlight"}
					, 100 as _matchPos
				FROM
					_search
				LEFT OUTER JOIN
					_rankFactors ON _search.docid = _rankFactors._id
				WHERE
					${this.kindFilter}
					AND _search MATCH ?
			`);
		...
		const prefixFTSQuery = query.allowsPrefixSearch ? buildFTSMatchString(query, true) : null;
		if (prefixFTSQuery) {
			searchQueries.push(`
				SELECT
					docid
				...
				WHERE
					...
					AND _search MATCH ?
			`);
			searchParams.push(prefixFTSQuery);
			...
			countQueries.push(`
				...
				WHERE
					...
					AND _search MATCH ?
			`);
			countParams.push(prefixFTSQuery);
		}

		if (!searchQueries.length) return this.constructor.emptyResult(page, perPage);

		const total = await this._db.get(`
			SELECT
				COUNT(*) as _total
			FROM
				_things
			JOIN(
				${countQueries.join(" UNION ")}
			) AS ranktable ON(ranktable.docid = _things._id)
			WHERE
				_status = ''
		`, countParams);

		const models = await this._db.all(`
			SELECT
				${this._reqAttrList}
				, _rank
				, _highlight
			...
			ORDER BY
				_matchPos ASC
				, ranktable._rank DESC
				, _filename COLLATE NOCASE ASC
			LIMIT ?
			OFFSET ?
			;
		`, searchParams.concat([perPage, (page - 1) * perPage]));
...
MaxBaseModel.STATUS_FILTER = "_status = ''";

module.exports = MaxBaseModel;
...
var mime = __webpack_require__(53)
...
const { join, normalize } = __webpack_require__(2);
const { FILE_KINDS } = __webpack_require__(8);

module.exports.PER_PAGE = 30;
module.exports.STATUS_FILTER = "_status = ''";
module.exports.FILE_ICON_PATH =  true ? normalize(join(__dirname, "..", "fileicons")) : undefined;

const ESCAPE_CHAR = "\\";
module.exports.ESCAPE_CHAR = ESCAPE_CHAR;
module.exports.ESCAPE_SUFFIX = `ESCAPE '${ESCAPE_CHAR}'`;

module.exports.C74_WEBSITE_BASE = "https://cycling74.com";
module.exports.C74_WEBSITE_API_URL = "https://cycling74.com/api/v1";
module.exports.C74_PACKAGE_API_URL = "https://package.cycling74.com/v1";
module.exports.C74_PACKAGE_MEDIA_PREFIX = "https://packageserver.s3.amazonaws.com";

module.exports.FILE_ICON_LOOKUP = new Map([[FILE_KINDS.GENOBJECT_COMMON, "gendsp_icon.png"], [FILE_KINDS.GENOBJECT_DSP, "gendsp_icon.png"], [FILE_KINDS.GENOBJECT_JIT, "genjit_icon.png"], [FILE_KINDS.GENDSP, "gendsp_icon.png"], [FILE_KINDS.GENEXPR, "genexpr_icon.png"], [FILE_KINDS.GENJIT, "genjit_icon.png"], [FILE_KINDS.OBJECT, "object_icon.png"], [FILE_KINDS.FAKEOBJECT, "object_icon.png"], [FILE_KINDS.CLIPPING, "clipping_icon.png"], [FILE_KINDS.PATCHER, "patcher_icon.png"], [FILE_KINDS.PROJECT, "project_icon.png"], [FILE_KINDS.SNIPPET, "snippetfile_icon.png"], [FILE_KINDS.PROJECT, "project_icon.png"], [FILE_KINDS.SNIPPET, "snippetfile_icon.png"], [FILE_KINDS.GUIDE, "vignette_icon.png"], [FILE_KINDS.TOPIC, "topic_icon.png"], [FILE_KINDS.TUTORIAL, "tutorial_icon.png"], [FILE_KINDS.VIGNETTE, "vignette_icon.png"], [FILE_KINDS.AMXD, "maxforlive_icon.png"], [FILE_KINDS.AUDIOPLUGIN, "audioplugin_icon.png"],

// File Icons...
[FILE_KINDS.FILE, "file_icon.png"]]);
...
const { Database, OPEN_READWRITE } = __webpack_require__(89);
const { existsSync, lstatSync } = __webpack_require__(10);
class C74DB {
	...
	constructor({
		...
		if (!path) throw new Error("Missing Database filepath");
		if (!existsSync(path)) throw new Error(`Invalid Database filepath. File ${path} does not exist`);
		if (lstatSync(path).isDirectory()) throw new Error("SQLite Path is a directory not a file.");
	...
	async searchTablesExist() {
		const info = await this.get("SELECT name FROM sqlite_master WHERE type='table' AND name='_search'");
		return !!info;
	}

	connect() {
		return new Promise((resolve, reject) => {
			if (this._db) return void resolve();

			const db = new Database(this._path, this._mode, async err => {
	...
	all(sql, params) {
		return new Promise((resolve, reject) => {
			if (!this._db) return void reject(new Error("Database closed."));
	...
	each(sql, params = [], fct) {
		return new Promise((resolve, reject) => {
			if (!this._db) return void reject(new Error("Database closed."));
			this._db.each(sql, params, fct, (err, noOfRows) => {
				if (err) return void reject(err);
				resolve(noOfRows);
			});
		});
...
module.exports = new C74DB({ path: process.env.DB_PATH });
...
var toIdentifier = __webpack_require__(45)
...
var formats = __webpack_require__(55);
...
var encodeUrl = __webpack_require__(30)
var escapeHtml = __webpack_require__(31)
var etag = __webpack_require__(62)
var fresh = __webpack_require__(63)
var fs = __webpack_require__(10)
var mime = __webpack_require__(148)
...
lazyProperty(module.exports, 'callSiteToString', function callSiteToString () {
  ...
  return stack[0].toString ? toString : __webpack_require__(99)
})

lazyProperty(module.exports, 'eventListenerCount', function eventListenerCount () {
  return EventEmitter.listenerCount || __webpack_require__(100)
})
...
var db = __webpack_require__(129)
...
var Layer = __webpack_require__(58);
...
var pathRegexp = __webpack_require__(139);
...
var forwarded = __webpack_require__(151)
var ipaddr = __webpack_require__(152)
...
const os = __webpack_require__(184);
...
var URL = url.URL;
var http = __webpack_require__(12);
var https = __webpack_require__(77);
var assert = __webpack_require__(67);
var Writable = __webpack_require__(17).Writable;
var debug = __webpack_require__(205)("follow-redirects");
...
const { C74_WEBSITE_BASE, C74_WEBSITE_API_URL, PER_PAGE } = __webpack_require__(23);
const { ensureQuery } = __webpack_require__(0);
const { badRequest, serverUnavailable } = __webpack_require__(1);
const { get } = __webpack_require__(72);

const websiteTypeToListBaseURL = type => {
	const url = [C74_WEBSITE_BASE];
...
class WebsiteBaseModel {

	static parse(data) {

		const { doc, highlight } = data;

		const listBase = websiteTypeToListBaseURL(doc.type);
		...
		return {
			...
			forum_topic: doc.forum_topic || "",
			excerpt: doc.excerpt || {},

			reply_count: doc.replies.length,
			tags: doc.tags,
			title: doc.title,

			type: doc.type,
			url: `${listBase}/${doc.name}`,
	...
	async search(query, page = 1, perPage = PER_PAGE) {
		if (!ensureQuery(query)) throw badRequest("Missing param 'query' for searching website posts.");
		if (!page) throw badRequest("Missing or invalid param 'page' for searching website posts.");
		if (!perPage) throw badRequest("Missing or invalid param 'perPage' for website posts.");

		try {
			// TODO: include tutorials and interviews by updating the POW API to allow multiple type queries...
			const response = await get(`${C74_WEBSITE_API_URL}/advancedsearch`, {
				responseType: "json",
				params: {
					q: query.lowerCaseText,
					perPage,
					pageNum: page,
					type: this.type
				}
			...
			for (let i = 0, il = results.length; i < il; i++) {
				const post = this.constructor.parse(results[i]);
			...
			throw serverUnavailable("Search API temporarily unavailable", { details: e.message });
		}
	}
}

module.exports = WebsiteBaseModel;

/***/ }),
/* 83 */
/***/ (function(module, exports, __webpack_require__) {

const program = __webpack_require__(84);
const PKG_INFO = __webpack_require__(86);

process.title = "Max Search Helper";

let DB_PATH = null;
let HTTP_PORT = null;

program.version(PKG_INFO.version, "-V, --version").description(PKG_INFO.description).option("--verbose", "Enables debug logging").option("--from-max", "Starts the Search Server with node.script communication capabilities.").option("--debug", "Enables DEBUG mode including logging output and runtime client build").arguments("<db_path> [port]").action((dbPath, port) => {
	DB_PATH = dbPath;
	HTTP_PORT = port || 8080;
});
...
// Are we running from within Max / node.script?!
const MaxAPI = program.fromMax ? __webpack_require__(87) : false;

if (!DB_PATH) throw new Error("Missing argument for SQLite Database path.");
process.env.DB_PATH = DB_PATH;

if (!HTTP_PORT) throw new Error("Missing argument for HTTP Port to listen on.");

// Enable DEBUG and DEV Mode
if (program.verbose) process.env.DEBUG = "c74:search";

const { shutdown, startup } = __webpack_require__(88);

let shuttingDown = false;
const shutdownServer = async (err, signal) => {
	if (shuttingDown) return;

	shuttingDown = true;

	if (MaxAPI && (program.verbose || err)) {
		// log if verbose OR if we got an error
		try {
			await MaxAPI.post(err ? `Search Server error occured: ${err.message}` : `Search server shutting down with signal: ${signal || 0}`);
		} catch (e) {}
	}

	if (signal && MaxAPI) {
		try {
			await MaxAPI.outlet("shutdown", signal);
		} catch (e) {}
	}
	await shutdown(signal ? signal : err ? 1 : 0, err);
};

const startupServer = async () => {
	try {
		const { port } = await startup(HTTP_PORT);
		HTTP_PORT = port;
		if (MaxAPI) {
			MaxAPI.outlet("port", HTTP_PORT);
			if (program.verbose) await MaxAPI.post(`Started Max 8 search server on port: ${HTTP_PORT}`);
		}
	} catch (e) {
		await shutdownServer(e);
	}
};

// Handle Uncaught Exceptions and try to shutdown gracefully
process.on("uncaughtException", shutdownServer);
process.on("unhandledRejection", shutdownServer);

// listen for TERM signal .e.g. kill
process.on("SIGTERM", () => shutdownServer(null, "SIGTERM"));

// listen for INT signal e.g. Ctrl-C
process.on("SIGINT", () => shutdownServer(null, "SIGINT"));
...
__webpack_require__(11).inherits(Command, EventEmitter);
...
module.exports = JSON.parse("{\"author\":\"Cycling '74\",\"contributors\":[{\"name\":\"Florian Demmer\",\"email\":\"florian@cycling74.com\"}],\"dependencies\":{\"@c74-search/max-constants\":\"1.0.0-alpha.1\",\"axios\":\"^0.17.1\",\"boom\":\"^7.1.1\",\"commander\":\"^2.15.1\",\"cors\":\"^2.8.4\",\"debug\":\"^3.1.0\",\"express\":\"^4.16.3\",\"immutable\":\"^3.8.2\",\"sqlite3\":\"Cycling74/node-sqlite3#c74-search-build\"},\"devDependencies\":{\"babel-core\":\"^6.26.0\",\"babel-loader\":\"^7.1.2\",\"node-loader\":\"^0.6.0\",\"webpack\":\"^4.10.2\",\"webpack-cli\":\"^3.3.10\",\"webpack-dev-middleware\":\"^3.1.3\",\"webpack-node-externals\":\"^1.7.2\"},\"description\":\"Prototypal NodeJS Server for unified Search in Max\",\"main\":\"c74.search.js\",\"name\":\"@c74-search/api-server\",\"private\":true,\"scripts\":{\"prebuild\":\"rm -rf dist\",\"build\":\"webpack --config ./webpack/dist.config.js\",\"postbuild\":\"node ./scripts/copySQLBinaries.js\"},\"version\":\"1.0.3\",\"n4mSettings\":{\"ABI_VERSION\":\"72\"}}");

/***/ }),
/* 87 */
/***/ (function(module, exports) {

module.exports = require("max-api");
...
const db = __webpack_require__(25);
const server = __webpack_require__(96);
const debug = __webpack_require__(68)("c74:search");

const shutdown = async (codeOrSignal = 0, err) => {
	if (err) debug(err);
	debug("Shutting down");
	...
	try {
		...
		debug("Error while attempting to shut down", e);
	}
...
module.exports.startup = async port => {

	try {
		await db.connect();
		const listenerPort = await server.listen(port);
		return { port: listenerPort };
	} catch (e) {
		debug("Error while starting up process");
		debug(e.message);
		throw e;
	}
};
...
var binding = process.platform === "darwin" ? 
    ...
    : process.platform === "win32" ? __webpack_require__(91)
    : __webpack_require__(92)(`./node-v72-${process.platform}-x64/node_sqlite3.node`);
...
sqlite3.verbose = function() {
    if (!isVerbose) {
        var trace = __webpack_require__(95);
        [
            'prepare',
            'get',
            'run',
...
module.exports = require("./sqlite3_darwin-x64.node");

/***/ }),
/* 91 */
/***/ (function(module, exports) {

module.exports = require("./sqlite3_win32-x64.node");
...
var map = {
	"./node-v72-darwin-x64/node_sqlite3.node": 93,
	"./node-v72-win32-x64/node_sqlite3.node": 94
};
...
const cors = __webpack_require__(165);
const { badImplementation, notFound } = __webpack_require__(1);

const apiRouter = __webpack_require__(170);
const debug = __webpack_require__(68)("c74:search");

const Models = __webpack_require__(186);

class Server {

	constructor(options = {}) {

		if (!options) throw new Error("Missing Server Configuration Object");
		...
		// In DEV Mode we enable webpack support
		if (false) {}
		...
		app.use((err, req, res, next) => {
			...
			let { output } = err;
			if (!output || err.isServer) debug(err.message, err.stack, err.data && err.data.details ? err.data.details : "");

			// Ensure to respond with a 500 Server Error if we encountered an uncaught internal error
			if (!output) output = badImplementation().output;
	...
	_bindServer(port) {
		return new Promise((resolve, reject) => {
			this._server.prependOnceListener("error", e => {
	...
	async listen(port) {
		if (!this._server) throw new Error("The server has been destroyed");
		if (this._server.listening) return this._port;

		let listeningPort = port || this._port;
		this._port = null;
		if (!listeningPort) throw new Error("Missing Port.");

		let tryCounter = 0;
		while (!this._port) {
			try {
				tryCounter++;
				const actualPort = await this._bindServer(listeningPort);
				this._port = actualPort;
			} catch (e) {
				if (tryCounter < 50) {
					debug(`Error while allocating port ${listeningPort} (${e.code}). Incrementing and retrying`);
					listeningPort++;
				} else {
		...
		process.env.HTTP_PORT = this._port;
		return this._port;
	}
	...
	destroy() {
		return new Promise((resolve, reject) => {
			...
			if (!this._server) return void resolve();
...
var proto = __webpack_require__(137);
var Route = __webpack_require__(57);
var Router = __webpack_require__(56);
var req = __webpack_require__(154);
var res = __webpack_require__(161);
...
exports.static = __webpack_require__(164);
...
function loadParser (parserName) {
  ...
  switch (parserName) {
    case 'json':
      ...
      break
    case 'raw':
      parser = __webpack_require__(131)
      break
    case 'text':
...
var read = __webpack_require__(28)
...
var createError = __webpack_require__(27)
var iconv = __webpack_require__(48)
var unpipe = __webpack_require__(51)
...
module.exports = {
    ...
    'gbk': {
        type: '_dbcs',
        table: function() { return __webpack_require__(36).concat(__webpack_require__(49)) },
    },
    'xgbk': 'gbk',
    'isoir58': 'gbk',
    ...
    'big5hkscs': {
        type: '_dbcs',
        table: function() { return __webpack_require__(50).concat(__webpack_require__(124)) },
...
var finalhandler = __webpack_require__(138);
var Router = __webpack_require__(56);
var methods = __webpack_require__(39);
var middleware = __webpack_require__(140);
...
function View(name, options) {
  ...
  if (!opts.engines[this.ext]) {
    ...
    var fn = __webpack_require__(142)(mod).__express
...
mime.define(__webpack_require__(149));
...
var accepts = __webpack_require__(155);
var deprecate = __webpack_require__(7)('express');
var isIP = __webpack_require__(47).isIP;
var typeis = __webpack_require__(21);
var http = __webpack_require__(12);
var fresh = __webpack_require__(63);
var parseRange = __webpack_require__(64);
...
var Negotiator = __webpack_require__(156)
var mime = __webpack_require__(53)
...
function loadModule(moduleName) {
  ...
  switch (moduleName) {
    case 'charset':
      module = __webpack_require__(157);
      break;
    case 'encoding':
      module = __webpack_require__(158);
      break;
    case 'language':
...
var statuses = __webpack_require__(20)
var merge = __webpack_require__(34);
var sign = __webpack_require__(162).sign;
...
var cookie = __webpack_require__(163);
var send = __webpack_require__(40);
var extname = path.extname;
var mime = send.mime;
var resolve = path.resolve;
var vary = __webpack_require__(66);
...
const Assert = __webpack_require__(67);
const Crypto = __webpack_require__(41);
const Path = __webpack_require__(2);

const DeepEqual = __webpack_require__(168);
const Escape = __webpack_require__(169);
...
[{ name: "docs", router: __webpack_require__(171) }, { name: "examples", router: __webpack_require__(172) }, { name: "files", router: __webpack_require__(173) }, { name: "fileicons", router: __webpack_require__(174) }, { name: "forums", router: __webpack_require__(175) }, { name: "gen", router: __webpack_require__(176) }, { name: "objects", router: __webpack_require__(177) }, { name: "packages", router: __webpack_require__(178) }, { name: "patchers", router: __webpack_require__(179) }, { name: "plugins", router: __webpack_require__(180) }, { name: "posts", router: __webpack_require__(181) }].forEach(({ name, router }) => {
...
const { Router } = __webpack_require__(3);
const { badImplementation, isBoom } = __webpack_require__(1);
const { parsePageQueryArg } = __webpack_require__(0);
const { localResourceCaching } = __webpack_require__(9);

const Query = __webpack_require__(6);

const router = new Router({});
router.use(localResourceCaching);

router.get("/search/:query", async (req, res, next) => {

	try {
		const docs = await req.locals.models.MaxDoc.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
		return void res.json(docs);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		const doc = await req.locals.models.MaxDoc.getById(req.params.id);
...
router.get("/search/:query", async (req, res, next) => {
	...
	try {
		const patchers = await req.locals.models.MaxExample.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
		return void res.json(patchers);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		const patch = await req.locals.models.MaxExample.getById(req.params.id);
		return void res.json(patch);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/search/:query", async (req, res, next) => {
	...
	try {
		const files = await req.locals.models.MaxFile.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		const file = await req.locals.models.MaxFile.getById(req.params.id);
...
const { FILE_KINDS } = __webpack_require__(8);
const { FILE_ICON_LOOKUP, FILE_ICON_PATH } = __webpack_require__(23);
const { getAPIBaseURL } = __webpack_require__(0);
const db = __webpack_require__(25);
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		...
		if (!row) return void next(notFound(`No such file '${id}'.`));

		// custom icon
		if (row._icon) {
			res.setHeader("Content-Type", "image/png");
			res.setHeader("Cache-Control", "public, max-age=31557600");
			res.end(Buffer.from(row._icon, "base64"));
			return;
		}

		const fkind = row._kind !== "vignette" ? row._kind : /_topic$/.test(row._name) ? FILE_KINDS.TOPIC : FILE_KINDS.GUIDE;
		res.redirect(302, `${getAPIBaseURL()}/fileicons/kinds/${encodeURIComponent(fkind)}`);
	} catch (e) {
		next(badImplementation("Interval Server Error", { details: e.message }));
	}
});

router.get("/kinds/:kind", async (req, res, next) => {

	let { kind } = req.params;
	if (!kind) return void next(notFound());

	// default to FILE icon
	if (!FILE_ICON_LOOKUP.has(kind)) kind = FILE_KINDS.FILE;

	res.sendFile(join(FILE_ICON_PATH, FILE_ICON_LOOKUP.get(kind)), {
		headers: {
			"Content-Type": "image/png",
			"Cache-Control": "public, max-age=31557600"
		}
	}, err => {
		if (err) next(badImplementation("Interval Server Error", { details: err.message }));
	});
});
...
const { badImplementation, isBoom } = __webpack_require__(1);
const { parsePageQueryArg } = __webpack_require__(0);
const { remoteResourceCaching } = __webpack_require__(9);
const Query = __webpack_require__(6);

const router = new Router({});
router.use(remoteResourceCaching);

router.get("/search/:query", async (req, res, next) => {
	try {
		const threads = await req.locals.models.ForumThread.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
...
router.get("/search/:query", async (req, res, next) => {
	...
	try {
		const files = await req.locals.models.MaxGen.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
		return void res.json(files);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		const file = await req.locals.models.MaxGen.getById(req.params.id);
		return void res.json(file);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
...
router.get("/search/:query", async (req, res, next) => {
	try {
		const objs = await req.locals.models.MaxObject.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
		return void res.json(objs);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/:name", async (req, res, next) => {
	try {
		const obj = await req.locals.models.MaxObject.getByName(req.params.name);
		return void res.json(obj);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/search/:query", async (req, res, next) => {
	try {
		const packages = await req.locals.models.MaxPackage.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
...
router.get("/search/:query", async (req, res, next) => {
	...
	try {
		const patchers = await req.locals.models.MaxPatcher.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
		return void res.json(patchers);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		const patch = await req.locals.models.MaxPatcher.getById(req.params.id);
		return void res.json(patch);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
...
router.get("/search/:query", async (req, res, next) => {
	...
	try {
		const plugins = await req.locals.models.AudioPlugin.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
		return void res.json(plugins);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/:id", async (req, res, next) => {
	...
	try {
		const plugin = await req.locals.models.AudioPlugin.getById(req.params.id);
		return void res.json(plugin);
	} catch (e) {
		next(isBoom(e) ? e : badImplementation("Internal Server Error", { details: e.message }));
	}
});
...
router.get("/search/:query", async (req, res, next) => {
	try {
		const posts = await req.locals.models.Post.search(new Query(req.params.query), parsePageQueryArg(req.query.page));
...
module.exports = [__webpack_require__(187), __webpack_require__(188), __webpack_require__(189), __webpack_require__(190), __webpack_require__(191), __webpack_require__(192), __webpack_require__(193), __webpack_require__(194), __webpack_require__(216), __webpack_require__(217)].reduce((sum, model) => {
...
const BaseModel = __webpack_require__(15);

class AudioPlugin extends BaseModel {
	...
	static parse(data) {
		return Object.assign(super.parse(data), {
			...
			url: `${getAPIBaseURL()}/plugins/${encodeURIComponent(data._id)}`
...
AudioPlugin.AUTOCOMPLETE_COLUMN = "_name";
AudioPlugin.NAME = "AudioPlugin";
AudioPlugin.REQ_ATTR = Object.freeze(["_desc", "_name"]);

AudioPlugin.KINDS = Object.freeze([FILE_KINDS.AMXD, FILE_KINDS.AUDIOPLUGIN]);

AudioPlugin.SEARCH_WEIGHTS = Object.freeze({
...
module.exports = new AudioPlugin();
...
class MaxDoc extends BaseModel {
	...
	static parse(data) {
		return Object.assign(super.parse(data), {
			...
			name: data._name,
			title: data._desc,
			url: `${getAPIBaseURL()}/docs/${encodeURIComponent(data._id)}`
...
MaxDoc.NAME = "MaxDoc";
MaxDoc.HIGHLIGHT = true;
MaxDoc.REQ_ATTR = Object.freeze(["_desc", "_name"]);

MaxDoc.KINDS = Object.freeze([FILE_KINDS.TUTORIAL, FILE_KINDS.VIGNETTE]);

MaxDoc.SEARCH_WEIGHTS = Object.freeze({
...
class MaxFile extends BaseModel {

	static parse(data) {
		return Object.assign(super.parse(data), {

			url: `${getAPIBaseURL()}/files/${encodeURIComponent(data._id)}`
...
MaxFile.AUTOCOMPLETE_COLUMN = "_filename";
MaxFile.NAME = "MaxFile";
MaxFile.REQ_ATTR = Object.freeze([]);

MaxFile.KINDS = Object.freeze(["java", "javascript", "lua", "material", "model", "pass", "shader", "json", "stylesheet", "textfile", "webpage", "xmlfile", "yaml", "audiofile", "midifile", "moviefile", "imagefile", "vectorimagefile"]);

MaxFile.SEARCH_WEIGHTS = Object.freeze({});

module.exports = new MaxFile();
...
class MaxGen extends BaseModel {
	...
	static parse(data) {
		return Object.assign(super.parse(data), {
			...
			name: data._name,
			description: data._desc,
			url: `${getAPIBaseURL()}/gen/${encodeURIComponent(data._id)}`
...
MaxGen.AUTOCOMPLETE_COLUMN = "_name";
MaxGen.NAME = "MaxGen";
MaxGen.REQ_ATTR = Object.freeze(["_desc", "_name"]);

MaxGen.KINDS = Object.freeze([FILE_KINDS.GENOBJECT_COMMON, FILE_KINDS.GENOBJECT_DSP, FILE_KINDS.GENOBJECT_JIT, FILE_KINDS.GENDSP, FILE_KINDS.GENEXPR, FILE_KINDS.GENJIT]);

MaxGen.SEARCH_WEIGHTS = Object.freeze({
...
class MaxObject extends BaseModel {
	...
	static parse(data) {
		return Object.assign(super.parse(data), {
			...
			name: data._name,
			description: data._desc,
			url: `${getAPIBaseURL()}/objects/${encodeURIComponent(data._name)}`
	...
	async getByName(name = null) {

		if (!name) throw badRequest("Missing 'name' parameter for requesting model.");
		...
		if (!model) throw notFound(`No such ${this.constructor.name} with name '${name}'`);
		return this.constructor.parse(model);
	}
}

MaxObject.AUTOCOMPLETE_COLUMN = "_name";
MaxObject.NAME = "MaxObject";
MaxObject.REQ_ATTR = Object.freeze(["_desc", "_name"]);

MaxObject.KINDS = Object.freeze([FILE_KINDS.FAKEOBJECT, FILE_KINDS.OBJECT]);

MaxObject.SEARCH_WEIGHTS = Object.freeze({
...
class MaxPatcher extends BaseModel {

	static parse(data) {
		return Object.assign(super.parse(data), {

			description: data._desc,
			url: `${getAPIBaseURL()}/patchers/${encodeURIComponent(data._id)}`
...
MaxPatcher.AUTOCOMPLETE_COLUMN = "_filename";
MaxPatcher.NAME = "MaxPatcher";
MaxPatcher.REQ_ATTR = Object.freeze(["_desc"]);

MaxPatcher.KINDS = Object.freeze([FILE_KINDS.CLIPPING, FILE_KINDS.PATCHER, FILE_KINDS.PROJECT, FILE_KINDS.SNIPPET]);

// Custom Kind Filter in order to exclude snippets as well
MaxPatcher.KIND_FILTER = `(_kind IN (${[FILE_KINDS.CLIPPING, FILE_KINDS.PATCHER, FILE_KINDS.PROJECT, FILE_KINDS.SNIPPET].map(k => `'${k}'`).join(", ")}) AND _tags NOT LIKE "%c74example%")`;

MaxPatcher.SEARCH_WEIGHTS = Object.freeze({
...
class MaxExample extends BaseModel {

	static parse(data) {
		return Object.assign(super.parse(data), {

			description: data._desc,
			url: `${getAPIBaseURL()}/examples/${encodeURIComponent(data._id)}`
...
MaxExample.AUTOCOMPLETE_COLUMN = "_filename";
MaxExample.NAME = "MaxExample";
MaxExample.REQ_ATTR = Object.freeze(["_desc"]);

MaxExample.KINDS = Object.freeze([FILE_KINDS.PATCHER]);

// Custom Kind Filter in order to exclude snippets as well
MaxExample.KIND_FILTER = `(_kind = "${FILE_KINDS.PATCHER}" AND _tags LIKE "%c74example%")`;

MaxExample.SEARCH_WEIGHTS = Object.freeze({
...
const { FILE_KINDS } = __webpack_require__(8);
const { C74_PACKAGE_API_URL, C74_PACKAGE_MEDIA_PREFIX, PER_PAGE } = __webpack_require__(23);
const { ensureQuery } = __webpack_require__(0);
const { badRequest, serverUnavailable } = __webpack_require__(1);
const { get } = __webpack_require__(72);
...
class MaxWebPackage {
	...
	static parse(data) {
		return {
			...
			externals: data.filelist && data.filelist.externals ? data.filelist.externals : [],
			kind: FILE_KINDS.PACKAGE,
			...
			iconURL: data.media && data.media.icon ? /^http(s)?:/.test(data.media.icon) ? data.media.icon : `${C74_PACKAGE_MEDIA_PREFIX}${data.media.icon}` : null,
			url: `${C74_PACKAGE_API_URL}/packages/${encodeURIComponent(data.name)}`
		};
	}
	...
	async search(query, page = 1, perPage = PER_PAGE) {
		if (!ensureQuery(query)) throw badRequest("Missing param 'query' for searching Packages.");
		if (!page) throw badRequest("Missing or invalid param 'page' for searching Packages.");
		if (!perPage) throw badRequest("Missing or invalid param 'perPage' for Packages.");

		try {
			const response = await get(`${C74_PACKAGE_API_URL}/packages`, {
				responseType: "json",
				params: {
					page,
					perPage,
					op: "or",
					"searchParams[name]": query.lowerCaseText,
					"searchParams[description]": query.lowerCaseText,
					"searchParams[external]": query.lowerCaseText
				},
			...
			for (let i = 0, il = results.length; i < il; i++) {
				const pck = this.constructor.parse(results[i]);
				if (pck) packages.push(pck);
			}
			...
			return {
				...
				page,
				perPage,
				total: response.data.pageInfo.totalRecords || 0
			};
		} catch (e) {
			throw serverUnavailable("Search API temporarily unavailable", { details: e.message });
		}
...
MaxWebPackage.NAME = "MaxPackage";

module.exports = new MaxWebPackage();
...
var http = __webpack_require__(12);
var https = __webpack_require__(77);
var httpFollow = __webpack_require__(78).http;
var httpsFollow = __webpack_require__(78).https;
var url = __webpack_require__(32);
var zlib = __webpack_require__(52);
...
module.exports = __webpack_require__(79)(exports);
var formatters = module.exports.formatters;
/**
 * Map %j to `JSON.stringify()`, since no Web Inspectors do that by default.
 */
...
const WebsiteBaseModel = __webpack_require__(82);

class WebsiteForumThread extends WebsiteBaseModel {
...
// TODO: include tutorials and interviews by updating the POW API to allow multiple type queries...
WebsiteForumThread.TYPE = "forum";

WebsiteForumThread.NAME = "ForumThread";

module.exports = new WebsiteForumThread();
...
class WebsitePost extends WebsiteBaseModel {
...
// TODO: include tutorials and interviews by updating the POW API to allow multiple type queries...
WebsitePost.TYPE = "tutorial";
WebsitePost.NAME = "Post";

module.exports = new WebsitePost();
