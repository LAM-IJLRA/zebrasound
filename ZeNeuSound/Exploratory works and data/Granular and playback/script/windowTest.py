...
axTemp.legend(), axTemp.set_xlabel(r"$n$"), axTemp.set_ylabel(r"$w(n)$ (UA)"), axTemp.set_title("Enveloppe, time domain")
axfreq.legend(), axfreq.set_xlabel(r"$\nu$"), axfreq.set_ylabel(r"$|W(\nu)|$ (UA)"), axfreq.set_title("Enveloppe, frequency domain"), axfreq.set_xlim(-0.004, 0.004)
...
for ind in a :
    ...
    w = (np.sin(np.pi*n**ind/(N**ind)))**2.
    windowed = w*buffer
    plt.plot(n, windowed/np.max(abs(windowed)), label = "a = " + str(ind), linewidth=1.)
